...
// SUPPRIMÉ simple30 : Mode Démo complet (updateDemoProgress, startDemoMode, stopDemoMode, cycles A/B, etc.)
...
/* ═══════════════════════════════════════════════════════════════
        ...
        // SUPPRIMÉ simple30 : Variables démo (demoMode, demoDuration, demoTimers, demoPhase, demoProgress)
        ...
        function loadSettings() {
            const settings = {
                ...
                // simple30 : Panel toujours visible
                activePanel: localStorage.getItem('activePanel') !== 'false',
            ...
            // SUPPRIMÉ simple30 : demoDuration
            ...
            // SUPPRIMÉ simple30 : applyBandeauStates (tout toujours visible)
        ...
        // SUPPRIMÉ simple30 : applyBandeauStates (tout est toujours visible)
...
/* ═══════════════════════════════════════════════════════════════
        ...
        function createPatientCardChat(patient) {
            ...
            // v2.3 : ID badge déplacé dans genderAgeLine (voir plus bas)
            // DÉPLACÉ simple30 : Score badge relocalisé avant le titre PATHOLOGIES (voir plus bas)
            ...
            // ╔═══════════════════════════════════════════════════════════════
            // ║ v2.3 : ID patient inline dans genderAgeLine
            // ║ + ✏️ cliquable si base contient "007"
            // ╚═══════════════════════════════════════════════════════════════
            const idSpan = document.createElement('span');
            idSpan.className = 'info-badge-light';
            idSpan.textContent = patientId;
            genderAgeLine.appendChild(idSpan);
            
            if (typeof currentBase === 'string' && currentBase.includes('007')) {
                const editBtn = document.createElement('span');
                editBtn.className = 'edit-patient-btn';
                editBtn.textContent = '✏️';
                editBtn.title = `Modifier le patient ${patientId}`;
                editBtn.addEventListener('click', (e) => {
                    e.stopPropagation();
                    window.open(`majpats.html?base=${encodeURIComponent(currentBase)}&id=${patientId}`, '_blank');
                });
                genderAgeLine.appendChild(editBtn);
            }
            ...
            if (pathoSource) {
                ...
                if (patient.similarity_score !== undefined && patient.similarity_score !== null) {
                    ...
                    scoreBadge.title = score === 100 
                        ...
                        : `Similarité : ${score}%`;
                    pathoSection.appendChild(scoreBadge);
                }
                
                const pathoTitle = document.createElement('div');
                pathoTitle.className = 'patient-card-section-title';
                pathoTitle.style.display = 'inline-block';
        ...
        function createPatientItemClassique(patient) {
            ...
            if (pathoSource) {
                ...
                // ╔═══════════════════════════════════════════════════════════════
                // ║ simple30 : Score badge INLINE avant le titre PATHOLOGIES (mode Classique)
                // ╚═══════════════════════════════════════════════════════════════
                if (patient.similarity_score !== undefined && patient.similarity_score !== null) {
                    const score = patient.similarity_score;
                    const scoreBadge = document.createElement('span');
        ...
        function initElements() {
            ...
            elements = {
                ...
                // SUPPRIMÉ simple30 : searchToolbarToggle, headerToolbar
                ...
                // SUPPRIMÉ simple30 : settingsButton
                ...
                // SUPPRIMÉ simple30 : demoToggle, demoProgressRing
            };
        ...
        window.addEventListener('DOMContentLoaded', async () => {
            ...
            // SUPPRIMÉ simple30 : baseSelectorSettings (pas de page paramètres)
            // ═══════════════════════════════════════════════════════════════
            // FIN BASE CHANGE v5.1
            // ═══════════════════════════════════════════════════════════════
            
            // SUPPRIMÉ simple30 : Toolbar toggle (tout toujours visible)
            
            // Toggle thème
            elements.themeToggle.addEventListener('click', toggleTheme);
            
            // SUPPRIMÉ simple30 : settingsButton navigation vers webparams.html
            ...
            // SUPPRIMÉ simple30 : demoToggle event listener
...
/* ═══════════════════════════════════════════════════════════════
        ...
        // ╔════════════════════════════════════════════════════════════════
        // ║ INITIALISATION simple30 - Tout toujours visible
        // ╚════════════════════════════════════════════════════════════════
        
        document.addEventListener('DOMContentLoaded', function() {
            console.log('🚀 Kitview Search simple30 initialisé');
            // simple30 : Pas de gestion bandeau, tout est toujours visible
        });

        // SUPPRIMÉ simple30 : Synchronisation inter-onglets avec webparams.html
        // (pas de page paramètres dans simple30)
