/**
 * main.js V2.5.3 - 30/01/2026
 * Kitview Search - Code principal (architecture modulaire)
 * 
 * CHANGEMENTS V2.5.3 :
 *   - FIX : Suppression ligne bleue séparatrice avant "Demander à l'IA"
 * 
 * CHANGEMENTS V2.5.2 :
 *   - FIX MODE CLASSIQUE : Ajout section "Demander à l'IA" dans le détail expandé
 *   - Uniformisation complète entre createPatientCardChat et createPatientItemClassique
 *   - Le bouton IA apparaît maintenant dans les deux modes
 * 
 * CHANGEMENTS V2.5.1 :
 *   - FIX PAGINATION : Cherche session_id dans response OU dans item
 *   - Debug : affiche où session_id a été trouvé
 * 
 * CHANGEMENTS V2.5.0 :
 *   - PAGINATION OPTIMISÉE : Utilise /search/page au lieu de /search
 *   - Latence pagination réduite de ~1500ms à ~50ms (~30x plus rapide)
 *   - Fallback vers /search si /search/page échoue (session expirée)
 *   - NOUVEAU FORMAT PAGINATION : "Page 3 / 321" au lieu de "41 à 60 sur 6407"
 *   - Le nombre total de résultats est affiché en haut (dans le message)
 * 
 * CHANGEMENTS V2.4.0 :
 *   - Toggle puces rondes : gestion des événements mode-pill
 *   - Mode Classique : pas de scroll smooth (remplacement instantané)
 *   - Compteur pagination : "41 à 60 sur 6407 - Page 3"
 *   - Footer unifié : Rating + Analyse IA sur une ligne
 *   - Affichage initial : 20 patients (était 100 dans certains cas)
 * 
 * CHANGEMENTS V2.3.0 :
 *   - FIX PAGINATION : Demande pageSize (20) au lieu de resultsLimit (100)
 *   - FIX PAGINATION : Mode Chat affiche pageSize patients à chaque clic
 *   - Cohérence : les deux modes avancent de pageSize à chaque clic
 * 
 * CHANGEMENTS V2.2.0 :
 *   - FIX PAGINATION : Endpoint /search au lieu de /trouve (404)
 *   - FIX PAGINATION : Gère le cas où pas de patients en mémoire
 *   - Fonction loadMorePatientsFromServer() centralisée
 * 
 * CHANGEMENTS V2.1.0 :
 *   - MODE CLASSIQUE UNIFORMISÉ avec mode Chat :
 *     • Même contenu : badges, pathologies groupées, tags, commentaires, IA
 *     • Chevron au lieu de loupe
 *     • Largeur 100% (pas de max-width)
 *   - PAGINATION GOOGLE (mode Classique) :
 *     • Remplace les résultats au lieu de les ajouter
 *     • Scroll en haut après changement de page
 *   - NOUVELLE RECHERCHE : Maintient le mode actuel (Chat ou Classique)
 * 
 * CHANGEMENTS V2.0.1 :
 *   - CHECKBOX FR EXTERNE : Suppression responseLangToggle
 *   - POPUP SIMPLIFIÉE : Clic direct sur chips
 * 
 * Ce fichier contient le code principal APRÈS extraction des modules :
 * - Fonctions de filigrane
 * - Mode démo
 * - Gestion des éléments DOM
 * - Rendu des patients (Chat et Classique uniformisés)
...
/* ═══════════════════════════════════════════════════════════════
        ...
        let currentMode = 'chat'; // 'chat' ou 'classique' (mode d'affichage UI)
