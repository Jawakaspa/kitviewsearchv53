...
fichiers_concernes: "server.py, search.py, trouve.py, detall.py, detia.py, detiabrut.py, detmeme.py, jsonsql.py, lancesql.py"
emoji_principal: "🔍"
-->
...
9. [Modules de Détection Spécialisés](#9-modules-de-détection-spécialisés)
   - 9.0 [detmeme.py - Similarités (V5.1)](#90-detmemepy---détection-des-similarités-v51)
   - 9.1 [detcount.py](#91-detcountpy---détection-listcount)
   - 9.2 [detangles.py](#92-detanglespy---angles-céphalométriques)
   - 9.3 [dettags.py](#93-dettagspy---tags-orthodontiques)
   - 9.4 [detadjs.py](#94-detadjspy---adjectifs-qualificatifs)
   - 9.5 [detage.py](#95-detagepy---âge-et-sexe)
...
13. [Annexes](#annexes--exemples)
    - A. [Exemples de détection detall.py](#annexe-a--exemples-de-détection-detallpy)
    - B. [Comparaison detall vs detia](#annexe-b--comparaison-detall-vs-detia)
    - C. [Trace d'exécution CLI](#annexe-c--trace-dexécution-avec-parcours)
    - D. [Structure BDD](#annexe-d--structure-de-la-base-de-données)
    - E. [Référentiels CSV](#annexe-e--référentiels-csv)
    - F. [Pourquoi detia utilise detmeme (algorithmique)](#annexe-f--pourquoi-detia-utilise-detmeme-algorithmique)
<!-- /NO_SLIDE -->
...
<!-- KEY: detall.py orchestre 6 modules spécialisés dans un ordre précis : detcount → detmeme → detangles → dettags → detage → motsvides -->

<!-- QUESTION: Pourquoi detmeme est-il appelé en deuxième position, avant la détection des pathologies ? -->

**Version** : 1.0.10 | **Auteur** : `cx`
...
└─────────┬───────────┘
          ...
          ▼
┌─────────────────────┐
│ ÉTAPE 1.5: detmeme  │  Similarités (même X)        ◄ NOUVEAU V5.1
│ "même portrait que" │  → critère type "meme"
└─────────┬───────────┘
          │ résidu
          ▼
┌─────────────────────┐
    ...
    Retourne:
        {
            ...
            'patterns_meme': {...},    # depuis communb.csv (V5.1)
...
<!-- TABLE
titre: "6 fichiers CSV de référence"
...
| `communb.csv` | Synonymes similarités (V5.1) | section;parametre;valeur;description |
...
| **Similarités (meme)** | **✓ via detmeme.py** | **✗ via detmeme.py** |

<!-- /TABLE -->

> **⚠️ Note importante** : Contrairement à detall.py, detia.py **n'intègre pas** la détection des similarités par IA. Quand une recherche de similarité est demandée (ex: "même portrait que Dupont"), c'est **detmeme.py** (algorithmique) qui est utilisé, même en mode IA. Voir [Annexe F](#annexe-f--pourquoi-detia-utilise-detmeme-algorithmique) pour les raisons de ce choix et les évolutions possibles.
...
<!-- KEY: 6 modules spécialisés traitent chacun un type de critère : count, meme (similarités), angles, tags+adjectifs, âge/sexe -->

---

<!-- SLIDE
id: "detmeme"
titre: "detmeme.py - Détection Similarités"
template: "schema"
emoji: "🔗"
timing: "4min"
transition: "slide"
...
### 9.0 detmeme.py - Détection des Similarités (V5.1)

<!-- KEY: detmeme détecte les expressions "même X que Y" pour la recherche de patients similaires -->

**Version** : 1.0.7 | **Entrée** : Résidu après detcount  
**Sortie** : Critères de type `"meme"` + référence patient

#### Principe

Détecte les demandes de recherche par similarité : "même portrait", "même âge", "même pathologie que Dupont".

#### Cibles détectées

<!-- TABLE
titre: "7 cibles de similarité"
colonnes_cles: "Cible,Colonne SQL"
style: "compact"
-->

| Mot-clé | Cible | Colonne SQL |
|---------|-------|-------------|
| tag, tags | tag | canontags |
| pathologie, pathologies | pathologie | canontags + canonadjs |
| portrait, photo | portrait | idportrait |
| sexe, genre | sexe | sexe |
| age, tranche d'âge | age | age (BETWEEN ±3) |
| nom, nom de famille | nom | nom |
| prenom | prenom | prenom |

<!-- /TABLE -->

#### Synonymes de "même" (depuis communb.csv)

`meme, memes, identique, similaire, commun, semblable, pareil`

#### Format de sortie JSON

<!-- CODE
langage: "json"
titre: "Critère de type meme"
executable: "false"
-->
```json
{
...
> **📘 Note** : Le fonctionnement détaillé de detmeme.py fait l'objet d'un document technique séparé.
...
<!-- KEY: Tous les modules de détection produisent le même format JSON avec auteur, listcount, critères, résidu et reference_meme (V5.1) -->
...
```json
{
    ...
    "residu": "",
    "question_originale": "même portrait que Guillaume Moulin, femmes avec béance antérieure de moins de 30 ans"
}
```
<!-- /CODE -->
...
| `meme` | Similarité (V5.1) | cible, valeur, label |
...
<!-- SLIDE
id: "annexe-f-detmeme-ia"
titre: "Annexe F : Pourquoi detia utilise detmeme (algorithmique)"
template: "2colonnes"
emoji: "🤔"
timing: "4min"
transition: "slide"
...
### Annexe F : Pourquoi detia utilise detmeme (algorithmique)

<!-- KEY: La détection des similarités reste algorithmique même en mode IA car c'est un pattern syntaxique simple, fiable et gratuit -->

#### Le choix actuel

Quand l'utilisateur fait une recherche de similarité en mode IA (ex: "même portrait que Dupont"), c'est **detmeme.py** (algorithmique) qui détecte le pattern, pas l'IA.

#### Raisons de ce choix

<!-- TABLE
titre: "Justification du choix algorithmique"
colonnes_cles: "Argument,Explication"
style: "large"
-->

| Argument | Explication |
|----------|-------------|
| **Pattern syntaxique simple** | "même X que Y" est un pattern régulier facilement détectable par regex |
| **Fiabilité proche de 100%** | Le vocabulaire est limité et contrôlé (meme, identique, similaire...) |
| **Coût nul** | Pas de consommation de tokens API pour une tâche triviale |
| **Latence minimale** | detmeme.py ajoute <5ms vs 500-2000ms pour un appel LLM |
| **Résultat prévisible** | Aucun risque d'hallucination sur la cible de similarité |

<!-- /TABLE -->

#### Limites de l'approche actuelle

- Ne comprend pas les reformulations créatives ("patients ressemblant à Dupont")
- Synonymes limités à ceux listés dans communb.csv
- Pas de compréhension contextuelle ("comme lui mais plus jeune")

---

<!-- SUBSLIDE
titre: "Évolution future : Similarités par IA"
parent: "annexe-f-detmeme-ia"
-->

#### Évolution future : Détection des similarités par IA

Si l'on décide à l'avenir que detia.py détecte également les similarités par IA, voici les modifications à apporter :

**1. Modification du prompt système (detia.py)**

<!-- CODE
langage: "python"
titre: "Ajout au prompt système"
executable: "false"
-->
```python
# Ajouter dans _construire_prompt_systeme() :

=== SIMILARITÉS (NOUVEAU) ===
Détecte les demandes de recherche par similarité.
Cibles possibles : tag, pathologie, portrait, sexe, age, nom, prenom

Format de sortie pour les similarités :
{
    "type": "meme",
    "cible": "portrait|pathologie|tag|sexe|age|nom|prenom",
    "valeur": "valeur spécifique ou null",
    "label": "Même portrait"
}

Si un patient de référence est mentionné :
{
    "reference": {
        "type": "nom|id",
        "valeur": "Nom Prénom ou ID"
    }
}

Exemples :
- "même portrait que Dupont" → cible=portrait, reference=Dupont
- "patients avec même pathologie" → cible=pathologie, reference=null
- "similaires à #123" → cible=tag, reference={type:id, valeur:123}
```
<!-- /CODE -->

**2. Modification de la fonction de mapping**

<!-- CODE
langage: "python"
titre: "Ajout dans _mapper_vers_canonique()"
executable: "false"
-->
```python
# Dans _mapper_vers_canonique(), ajouter :
if critere.get('type') == 'meme':
    # Valider que la cible est connue
    cibles_valides = {'tag', 'pathologie', 'portrait', 'sexe', 'age', 'nom', 'prenom'}
    if critere.get('cible') not in cibles_valides:
        critere['cible'] = 'tag'  # Fallback par défaut
    # Propager la référence si présente
    if resultat.get('reference'):
        detection['reference_meme'] = resultat['reference']
```
<!-- /CODE -->

**3. Choix de l'approche : Hybride ou Full-IA**

| Option | Avantages | Inconvénients |
|--------|-----------|---------------|
| **Hybride** (detmeme d'abord, IA en fallback) | Rapide pour patterns simples | Complexité du code |
| **Full-IA** (IA uniquement) | Code simplifié | Coût API, latence systématique |
| **Status quo** (detmeme seul) | Fiable, gratuit | Ne comprend pas les reformulations |

**Recommandation** : Conserver l'approche **hybride** en ajoutant la détection IA en fallback uniquement quand detmeme ne détecte rien mais que la question contient des mots évoquant la similarité.
...
3. **Similarités (V5.1)** : detmeme.py détecte les "même X que Y" (algorithmique uniquement)
4. **Format unifié** : JSON standardisé entre tous les modules
5. **Protection intégrée** : Garde-fou contre les requêtes sans filtres
6. **Multilingue natif** : 12 langues avec traduction bidirectionnelle

### Métriques de performance

- **detall.py** : ~5-20ms, gratuit
- **detmeme.py** : ~2-5ms, gratuit (V5.1)
...
*Document généré le 30/01/2026 - KITVIEW Search V5 Documentation Technique*
*Version 1.1.0 - Slides-Ready avec métadonnées Reveal.js*
*Nouveauté V5.1 : Intégration detmeme.py (détection similarités)*
