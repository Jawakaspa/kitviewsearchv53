# Prompt_detia_benchmark.md

## Mon avis sur l'architecture pour les tests "handicap"

### Les 3 options analysées

| Option                                 | Description                               | Avantages                             | Inconvénients                                        |
| -------------------------------------- | ----------------------------------------- | ------------------------------------- | ---------------------------------------------------- |
| **1. Intégrer dans detia**             | Ajouter des paramètres `--exclude-refs`   | Code centralisé, simple à utiliser    | Pollue le code principal, complexifie la maintenance |
| **2. Sur-ensemble qui importe detia**  | `detia_benchmark.py` qui appelle detia    | detia reste propre, flexibilité max   | Duplication potentielle, 2 fichiers à maintenir      |
| **3. Sous-ensemble importé par detia** | `detia_core.py` minimal, detia l'enrichit | Séparation claire des responsabilités | Refactoring important, risque de régression          |

### Ma recommandation : **Option 2 - Sur-ensemble**

Raisons :

1. **detia.py reste stable** - Pas de régression sur le code de production
2. **Besoin non pérenne** - Tu le dis toi-même, c'est pour des tests de benchmark
3. **Flexibilité** - Le sur-ensemble peut facilement manipuler les références avant de les passer à detia
4. **Isolation** - Si le benchmark plante, detia continue de fonctionner

### Architecture proposée

```
detia.py (inchangé sauf unification des noms de moteurs)
    ↑
    │ import
    │
detia_benchmark.py (nouveau)
    - Charge les références avec exclusions
    - Appelle detia.detecter_tout() avec références filtrées
    - Génère rapports comparatifs
```

Le `detia_benchmark.py` pourrait avoir une signature comme :

bash

```bash
python detia_benchmark.py "question" --model sonnet --exclude angles,ages
python detia_benchmark.py tests.csv --model gpt4o --exclude syntags --compare-with rap*
```
